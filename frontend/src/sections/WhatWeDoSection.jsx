            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                {services.map(({ title, description, icon, link }, index) => (
                    <article
                        ...
                        className={`group bg-blue-50 border border-blue-100 rounded-lg p-6 shadow-sm hover:shadow-lg hover:-translate-y-2 hover:bg-blue-100 transition-all duration-300 ease-in-out animate-fadeIn`}
                        style={{ animationDelay: `${index * 0.2}s`, animationFillMode: "both" }}
                    >
                        <div className="flex items-center gap-3 mb-4 text-blue-600 group-hover:scale-110 transition-transform duration-300">
                            <span className="text-3xl">{icon}</span>
                            <h3 className="text-xl md:text-2xl font-semibold line-clamp-1">{title}</h3>
                        </div>
